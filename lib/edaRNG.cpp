...
ulong edaRNG::tm = 1234567;

unsigned int edaRNG::random(unsigned int n)
{
  return (unsigned int) (n * uniform());
}

unsigned int edaRNG::random(unsigned int min, unsigned int max)
{
  if(min > max) swap (max, min);
  return (unsigned int) ( (max - min) * uniform() + min);
}

//________________________________________________________________________
// edaRNG::RNOR generates normal variates with rejection.
// nfix() generates variates after rejection in RNOR.
// Despite rejection, this method is faster than Box-Muller.

double edaRNG::nfix(slong h, ulong i)
{
...
// edaRNG::REXP generates exponential variates with rejection.
// efix() generates variates after rejection in REXP.

double edaRNG::efix(ulong j, ulong i)
{
  for (;;) {
    if (i == 0)
...
double edaRNG::gamma(double shape, double scale)
{
  if (shape < 1.)
...
int edaRNG::binomial(double pp, int n)
{
...
void edaRNG::multinom(uint n, const vector<double>& probs, vector<uint>& samp)
{
  samp.resize(probs.size());
  edaRNG::multinom(n, &probs[0], (uint) probs.size(), &samp[0]);
}

void edaRNG::multinom(uint size, const double* probs, uint num_probs, uint* samp)
{
