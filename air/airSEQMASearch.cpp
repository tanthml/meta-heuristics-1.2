...
int main(int argc, char** argv) {
  ...
  else
  {
    ...
    nullid = sfcontrol.insertVertex (&nullSearch);

    for(unsigned int i = 0; i < n; i++) {
      edaIDSelectWrapper slect(i);
      hcid[i] = sfcontrol.insertVertex (&hcSearch, slect);                         
    }
    gaid = sfcontrol.insertVertex (&gaSearch);   
    for(unsigned int i = 0; i < n; i++) {
      sfcontrol.insertEdge(nullid, hcid[i]);
      sfcontrol.insertEdge(hcid[i], gaid);
    }
    
    edaGenContinue loop(50);
    sfcontrol.insertLoop(gaid, nullid, loop);
