...
tspTwoOpt::tspTwoOpt(const Graph &g)
  : edaMove(), graph (new Graph(g))
{
}

tspTwoOpt::tspTwoOpt(const tspTwoOpt &move) : 
...
edaMove* tspTwoOpt::clone() const
{
  return new tspTwoOpt(*this);
}

void tspTwoOpt::init(const edaSolution &sol)
{
  this->first = 0;
  this->second = 2;
}

double tspTwoOpt::incrEval(const edaSolution &sol) const
{
...
void tspTwoOpt::update( edaSolution &sol ) const
{
...
void tspTwoOpt::Serialize( edaBuffer &buf, bool pack )
{
  if ( pack )
  {
...
edaMove& tspTwoOpt::operator = (const edaMove& _move)
{
  this->first = ((tspTwoOpt&)_move).first;
  this->second = ((tspTwoOpt&)_move).second;

  return (*this);
}

bool tspTwoOpt::operator == (const edaMove &_move) const
{
  return (*this) == _move;
}

void tspTwoOpt::printOn( ostream &os ) const
{
  os << "[" << this->first << "," << this->second << "]";
}

void tspTwoOpt::easer() {
  if (graph != NULL)
  {
    delete graph;
  } 
