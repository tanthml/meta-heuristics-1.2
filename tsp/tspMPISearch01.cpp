...
int main (int argc, char **argv)
{
  ...
  {    
    ...
    edaRouletteWheelSelection rank(0.5);    
    edaPartiallyMatchedCrossover cross;
    edaRandSwapMutation mute(0.05);   
    edaGA gaSearch(&cont, &repre, &rank, &cross, &mute);    
    
    // Use the declaration below to run on single machine:
    edaMpiWrapperControl sfControl; 
    edaFullSelectWrapper fSlect;
    
    int said0 = sfControl.insertVertex (&saSearch, fSlect);   
    int said1 = sfControl.insertVertex (&saSearch, fSlect);  
    int tsid2 = sfControl.insertVertex (&tsSearch, fSlect);
    int tsid3 = sfControl.insertVertex (&tsSearch, fSlect);
    int hcid4 = sfControl.insertVertex (&hcSearch, fSlect);
    int hcid5 = sfControl.insertVertex (&hcSearch, fSlect);
    int gaid6 = sfControl.insertVertex (&gaSearch, fSlect);
    
    sfControl.insertEdge(said0, hcid4);
    sfControl.insertEdge(hcid4, tsid2);
    sfControl.insertEdge(said1, hcid5);
    sfControl.insertEdge(hcid5, tsid3);
    sfControl.insertEdge(tsid3, gaid6);
    sfControl.insertEdge(tsid2, gaid6);
